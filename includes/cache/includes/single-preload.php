		...
		if( $term && !is_wp_error($term) ) {
			...
			if( self::is_mobile_active() ) {
				array_push(static::$urls, array(
					"url" => $url,
					"user-agent" => "WP Fastest Cache Preload iPhone Mobile Bot"
				));
			}
	...
	public static function put_inline_js()
	{
		...
		if( $screen->parent_base == "edit" && $screen->base == "post" ) {
			...
			<div id="wpfc-single-preload" class="notice notice-info is-dismissible" style="display: none;">
				<p id="wpfc-single-preload-info"><?php _e('Cache is generated for this content', 'wp-fastest-cache'); ?>
					<label style="display: none;" id="wpfc-single-preload-error">0</label><label id="wpfc-single-preload-process" style="padding-left: 5px;"><span>0</span>/<span><?php echo count(static::$urls); ?></span></label>
				</p>
